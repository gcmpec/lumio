...
export async function GET({ locals }) {
  ...
  try {
    const items = await service.listEligibleDeliverables();
    return new Response(JSON.stringify({ success: true, items }), {
      status: 200,
      headers: {
        "Content-Type": "application/json; charset=utf-8",
        "Content-Disposition": `attachment; filename="eligible-deliverables-${new Date().toISOString().slice(0, 10)}.json"`,
      },
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Não foi possível exportar os deliverables";
