...
export async function POST({ locals, request }) {
  ...
  try {
    ...
    if (!Array.isArray(items) || items.length === 0) {
      return Response.json({ success: false, message: "Indica pelo menos um engagement para importar" }, { status: 400 });
    }

    const result = await service.importEligibleEngagements(items);
    return Response.json({ success: true, ...result });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Não foi possível importar os engagements";
